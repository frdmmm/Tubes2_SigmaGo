document.getElementById('searchForm').addEventListener('submit', function(event) {
  ...
  let startn = start.replace(/ /g, "_");
  let endn = end.replace(/ /g, "_");

  const formData = new FormData();
  formData.append('start', startn);
  formData.append('end', endn);
  ...
  .then(data => {
    ...
    solution.forEach((node, index) => {
        ...
        nodeElement.href = `https://en.wikipedia.org/wiki/${encodeURIComponent(node)}`;
